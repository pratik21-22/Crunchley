  ...
  persist<CartState>(
    (set, get): CartState => ({
      ...
      addItem: (item, quantity = 1) => {
        ...
        set((state) => {
          const existingItem = state.items.find((i) => normalizeId(i.id) === normalizedId);
            ...
            return {
              items: state.items.map((i) =>
                normalizeId(i.id) === normalizedId
                  ? { ...i, id: normalizedId, quantity: i.quantity + safeQuantity }
                  : i
              ),
            };
          }
          ...
          // Add new item
          return { items: [...state.items, { ...item, id: normalizedId, quantity: safeQuantity }] };
        });
      },
      ...
      updateQuantity: (id, quantity) => {
        ...
        set((state) => ({
          items: state.items.map((i) =>
            normalizeId(i.id) === normalizedId
              ? { ...i, id: normalizedId, quantity: Math.max(1, quantity) }
              : i
          ),
        }));
      },
      ...
      increaseQuantity: (id) => {
        ...
        set((state) => ({
          items: state.items.map((i) =>
            normalizeId(i.id) === normalizedId
              ? { ...i, id: normalizedId, quantity: i.quantity + 1 }
              : i
          ),
        }));
      },
      ...
      decreaseQuantity: (id) => {
        ...
        set((state) => ({
          items: state.items.map((i) =>
            normalizeId(i.id) === normalizedId
              ? { ...i, id: normalizedId, quantity: Math.max(1, i.quantity - 1) }
              : i
          ),
        }));
      },
